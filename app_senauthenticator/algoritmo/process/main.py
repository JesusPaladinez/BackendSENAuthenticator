...
# Diseño de la interfaz del registro y el login
...
class GraphicalUserInterface:
    def __init__(self, root):
        self.main_window = root # Ventana principal
        self.main_window.title('SENAuthenticator') # Título de la ventana
        self.main_window.geometry('1280x720') # Dimensiones de la ventana
        self.frame = CustomFrame(self.main_window) # Empaqueta o agrupa los elementos de la ventana principal        
    ...
    def facial_login(self):
        if self.cap:
            ...
            if ret:
                ...
                # Esta parte se comenta porque es de la parte de electrónica
                # if user_access:
    ...
    def gui_login(self):
        ...
        self.face_login_window.title('Inicio de sesión facial')
    ...
    def data_sign_up(self):
        ...
        if len(self.name) == 0 or len(self.user_code) == 0:
            ...
            if self.user_code in self.user_codes:
                print('¡Ya hay un usuario registrado con ese número de documento!')
            else:
                # guardar datos
    ...
    def main(self):
        # background
        background_img = PhotoImage(file=self.images.init_img) # Imagen de fondo
